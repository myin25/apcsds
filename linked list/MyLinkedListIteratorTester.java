...
public class MyLinkedListIteratorTester
{
    ...
    public static void main(String[] args)
    {
        ...
        for(int i = 0; i < 500; i++){
            ...
            if(div == 0 || div == 1){
                ...
                for(int j = origSize - 1; j > (origSize / 2); j--){
                    ...
                    if(list.size() != orig.size())
                        throw new RuntimeException("Your list was size " + list.size() + "instead of " + orig.size());
                }
            } else if(div == 5){
                ...
                System.out.println("just to double check " + orig);
                ...
                for(int j = orig.size() - 1; j > 0; j--){
                    ...
                    if(list.size() != orig.size())
                        throw new RuntimeException("Your list is the wrong size (should have been " + orig.size() + "instead of" + list.size() + ").");
                }
            }
            System.out.println("Orig: " + orig.toString());
            System.out.println("Your List: " + list.toString());
            ...
            if (!list.toString().equals(orig.toString()))
                System.out.print(list.toString() + " " + orig.toString());
            if(listSum != origSum || !list.toString().equals(orig.toString()))
                throw new RuntimeException("brh Check your iterator and linked list implementation, and also your add*() and remove*() methods.");
        }
