...
public class SolitaireDisplay extends JComponent implements MouseListener
{
    ...
    private boolean foundationselected; // variable not in original code that returns if a foundation was clicked before a pile/waste
    private int selectedfoundationindex; // variable not in orignal code which returns the index of the selected foundation
    ...
    public void paintComponent(Graphics g)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            // draw a yellow border around a foundation card if it is selected
        ...
        // check to see if player has won or not
        if (game.won())
        {
            if (!new File("winscreen.gif").exists())
                throw new IllegalArgumentException("bad file name:  " + "winscreen.gif");
            Image image = new ImageIcon("winscreen.gif").getImage();
            g.drawImage(image, getWidth()/2, getHeight()/2, null); 
        }
    }
    ...
    /**
     * returns if a foundation was selected (before a pile/waste was selected)
     * @return true if foundation was selected, false if not
     */
    ...
    /**
     * selects a foundation and updates selectedfoundationindex to keep track of index of selected
     * @param index is the index of selected foundation stack (0, 1, 2, 3)
     */
    ...
    /**
     * returns the selected foundation's index
     * @return selected foundations index if a foundation is selected
     *          returns -1 if a foundation is not selected
     */
    public int selectedFoundationIndex()
    {
